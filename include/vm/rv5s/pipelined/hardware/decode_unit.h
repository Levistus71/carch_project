 ...
 */
class PipelinedDecodeUnit {
public:
    ~PipelinedDecodeUnit() = default;

    /**
     * @brief 
     */
    void DecodeInstruction(PipelinedInstrContext& instr_context, register_file::RegisterFile& rf);
...
private:
     ...
     */
    void DecodeInstrFields(PipelinedInstrContext& instr_context);
     ...
     */
    void SetRegImmValues(PipelinedInstrContext& instr_context, register_file::RegisterFile& rf);

    /**
     * @brief SetControlSignals sets the alu flags, mux select line flags.
     * @param instr_context
     */
    void SetContextValues(PipelinedInstrContext& instr_context);


    /**
     * @brief SetMemAccessSize sets the num_bytes field of PipelinedInstrContext. Just tells how many bytes are being accesed in the memory phase
     */
    void SetMemValues(PipelinedInstrContext& instr_context);

    /**
     * @brief GetAluSignal helper function used by SetContextValues which sets the alu flags
     * @param instr_context
     */
    alu::AluOp GetAluSignal(PipelinedInstrContext& instr_context);

    /**
     * @brief ImmGenerator is a helper function used by SetRegImmValues, generates the immediate
     * @param instr_context
     */
    int32_t ImmGenerator(PipelinedInstrContext& instr_context);
};
