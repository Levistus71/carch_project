...
#include "vm/rv5s/pipelined/hardware/decode_unit.h"
#include "vm/rv5s/pipelined/hardware/decode_helper.h"
...
void PipelinedDecodeUnit::DecodeInstruction(PipelinedInstrContext& instr_context, RegisterFile& rf){
    DecodeInstrFields(instr_context);
    SetRegImmValues(instr_context, rf);
    SetContextValues(instr_context);
    // this function should be called after SetContextValues(PipelinedInstrContext);
    SetMemValues(instr_context);
}


void PipelinedDecodeUnit::DecodeInstrFields(PipelinedInstrContext& instr_context){
...
void PipelinedDecodeUnit::SetRegImmValues(PipelinedInstrContext& instr_context, RegisterFile& rf){
...
void PipelinedDecodeUnit::SetMemValues(PipelinedInstrContext& instr_context){
...
void PipelinedDecodeUnit::SetContextValues(PipelinedInstrContext& instr_context) {
...
int32_t PipelinedDecodeUnit::ImmGenerator(PipelinedInstrContext& instr_context){
    int32_t imm = 0;
