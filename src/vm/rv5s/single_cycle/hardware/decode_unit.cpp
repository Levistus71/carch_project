...
#include "vm/rv5s/single_cycle/hardware/decode_unit.h"
#include "vm/rv5s/single_cycle/hardware/decode_helper.h"
...
void SingleCycleDecodeUnit::DecodeInstruction(SingleCycleInstrContext& instr_context, RegisterFile& rf){
    DecodeInstrFields(instr_context);
    SetRegImmValues(instr_context, rf);
    SetContextValues(instr_context);
    // this function should be called after SetContextValues(SingleCycleInstrContext);
    SetMemValues(instr_context);
}


void SingleCycleDecodeUnit::DecodeInstrFields(SingleCycleInstrContext& instr_context){
...
void SingleCycleDecodeUnit::SetRegImmValues(SingleCycleInstrContext& instr_context, RegisterFile& rf){
...
void SingleCycleDecodeUnit::SetMemValues(SingleCycleInstrContext& instr_context){
...
void SingleCycleDecodeUnit::SetContextValues(SingleCycleInstrContext& instr_context) {
...
int32_t SingleCycleDecodeUnit::ImmGenerator(SingleCycleInstrContext& instr_context){
    int32_t imm = 0;
