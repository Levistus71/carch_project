...
void TripleIssueStages::Decode(TripleIssueCore& vm_core){
	...
	if(!instr1.illegal){
        vm_core.decode_unit_.DecodeInstruction(instr1, vm_core.register_file_);
    ...
    if(!instr2.illegal){
        vm_core.decode_unit_.DecodeInstruction(instr2, vm_core.register_file_);
    ...
    if(!instr3.illegal){
        vm_core.decode_unit_.DecodeInstruction(instr3, vm_core.register_file_);
